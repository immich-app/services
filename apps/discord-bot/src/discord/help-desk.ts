...
import { type ArgsOf, ButtonComponent, Discord, ModalComponent, On, Slash, SlashChoice, SlashOption } from 'discordx';
import { Constants, DiscordButton, DiscordField, DiscordModal } from '../constants.js';
import { DiscordChannel } from '../interfaces/discord.interface.js';
import { createLogger } from '../util.js';

const logger = createLogger('DiscordHelpDesk');
...
export class DiscordHelpDesk {
  ...
  async handleSubmit(interaction: ButtonInteraction): Promise<void> {
    ...
    if (thread.appliedTags.includes(Tags.HelpDeskReady)) {
      return;
    }
  ...
  async handleThreadCreate([thread]: ArgsOf<'threadCreate'>) {
    ...
    try {
      ...
      logger.error('Retrying helpdesk welcome message:', error);
      setTimeout(() => void this.sendWelcomeMessage(user, thread), 5000);
    }
  }
