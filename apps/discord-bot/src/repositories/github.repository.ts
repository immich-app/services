...
import type { IGithubInterface, PullRequest } from '../interfaces/github.interface.js';
import { createLogger } from '../util.js';

const logger = createLogger('GithubRepository');
...
const handleGraphqlError = (error: unknown) => {
  ...
  if ((error as GraphqlResponseError<unknown>).errors?.[0]?.type !== 'NOT_FOUND') {
    throw error;
  }
};
...
export class GithubRepository implements IGithubInterface {
  ...
  async getIssueOrPr(org: string, repo: string, id: number) {
    try {
      ...
      logger.log(`Could not fetch issue or PR #${id}`);
    }
  }
  ...
  async getDiscussion(org: string, repo: string, id: number) {
    try {
      ...
      handleGraphqlError(error);
      logger.log(`Could not fetch discussion #${id}`);
    }
  }
  ...
  async getCheckSuiteTriggerCommit(_org: string, _repo: string, checkSuiteNodeId: string) {
