resource "cloudflare_d1_database" "version" {
  account_id = var.cloudflare_account_id
  name       = "version-releases${local.resource_suffix}"
...
resource "null_resource" "d1_migrations" {
  triggers = {
    database_id     = cloudflare_d1_database.version.id
  ...
  provisioner "local-exec" {
    command = <<-EOT
      for f in $(ls ${var.migrations_dir}/*.sql | sort); do
        ...
        curl -sf -X POST \
          "https://api.cloudflare.com/client/v4/accounts/${var.cloudflare_account_id}/d1/database/${cloudflare_d1_database.version.id}/query" \
  ...
  depends_on = [cloudflare_d1_database.version]
}
